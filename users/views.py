...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render_to_response('nuevo.html', {'title': 'Registro', 'formulario': form, 'state': state},
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            try:
                ...
                state = "Password o usuario incorrecto."
                return render_to_response('login.html', {'title': 'Login', 'state': state, 'username': username},
                                          context_instance=RequestContext(request))
...
def update(request):
    ...
    return render_to_response('profile.html',
                              {'title': 'Informacion personal', 'User': usuario, 'Extend': extendido, 'MERROR': error,
                               'Alias': pseudo}, context_instance=RequestContext(request))
...
def perfil(request, user_id):
    if not request.user.is_authenticated():
        ...
        return render_to_response('perfil.html', {'title': 'Perfil', 'user': usuario, 'User': usuario_, "tweets": post,
                                                  'follower': followers})
